...
FinishPage.prototype.storeParticipantData = function() {
  ...
  for (i = 0; i < this.pageConfig.questionnaire.length; ++i) {
      ...
      if (element.type === "multiple_choice") {
        var checked = $("input[name='"+element.name+"_response']:checked").map(function(){ return this.value; }).get();
        this.session.participant.response[this.session.participant.response.length] = checked.join(", ");
...
FinishPage.prototype.render = function (_parent) {
    ...
    for (i = 0; i < this.pageConfig.questionnaire.length; ++i) {
      ...
      if (element.type === "text") {
        ...
        table.append($("<tr><td id='labeltd' style='vertical-align:top; padding-top:"+ $('#feedback').css('margin-top') +"'><strong>"+ element.label +"</strong></td><td><textarea name='"+element.name+"' id='"+element.name+"' rows='4' style='width:100%;'></textarea></td></tr>"));
        ...
        for (var j = 0; j < element.response.length; ++j) {
          var opt = element.response[j];
          var id = element.name+"_response_"+j;
          group.append($("<input type='checkbox' value='"+opt.value+"' name='"+element.name+"_response' id='"+id+"'/> \
    ...
    this.interval = setInterval((function(){ 
      ...
      for (i = 0; i < this.pageConfig.questionnaire.length; ++i) {
        ...
        if (element.type === "text") {
          ...
          if ($("input[name='" + element.name + "_response']:checked").length > 0 || element.optional == true) {
            ++counter;
          }
        }
